...
<head>
<meta charset="UTF-8">
<link href="/group5/css/findPcCafe.css" rel="stylesheet" type="text/css" />
...
<jsp:include page="../outframe/NavBar2.jsp" />
	...
	<main>
		...
		cafeList = cDao.getGoogleCafe(pageNum, pointValue, passValue, payValue, eventValue);
		%>
		<div id="cafelist">
			<div id="cafe_inner">
				<table id="list">
					...
					<tr class="contents">
						...
						<%if(cafeList.get(i).getPoint().equals("TRUE")){ %>
							<img src="https://www.adinventory.co.kr/playgeto/assets/img/pcbang_find/icon_s1.png" style="width: 18px; height: 18px;"/>
						<%} %>
						<% if(cafeList.get(i).getPass().equals("TRUE")){ %>
							<img src="https://www.adinventory.co.kr/playgeto/assets/img/pcbang_find/icon_s2.png" style="width: 18px; height: 18px;"/>
						<%} %>
						<% if(cafeList.get(i).getPay().equals("TRUE")){ %>
							<img src="https://www.adinventory.co.kr/playgeto/assets/img/pcbang_find/icon_s3_1.png" style="width: 36px; heigth: 18px;" />
						<%}if(cafeList.get(i).getEvent().equals("TRUE")){ %>
							<img src="https://www.adinventory.co.kr/playgeto/assets/img/pcbang_find/icon_s4.png" style="width: 18px; height: 18px;" />
						<%} %>
						</td>
						<td style="width: 1000px;" class="address"><%= cafeList.get(i).getAddress() %></td>
						<td style="width: 100px; text-align: center;"><button class="find_btn"><span>위치보기</span></button></td>
					</tr>
					<%} %>
				</table>
				<%
					int totalPageNum = (int)Math.ceil((double)cafeList.get(0).getTotalnum()/10);
					
				%>
				<%if(cafeList.size()>=10){ %>
					<div class="buttons" style="position: relative; padding: 1rem 0; display: flex; justify-content: center;">
						<% if(totalPageNum>10 && minPage != 1){ %>
						<a href="FindGoogle.jsp?minPage=<%=minPage-10%>&pageNum=<%=minPage-10%>&pointValue=<%=pointValue%>&passValue=<%=passValue%>&payValue=<%=payValue%>&eventValue=<%=eventValue%>"> 이전 </a>
						<% } %>
						<%if(totalPageNum <=maxPage){ %>
							<%for(int i =1; i<=totalPageNum; i++){ %>
							<a href="FindGoogle.jsp?pageNum=<%=i%>&pointValue=<%=pointValue%>&passValue=<%=passValue%>&payValue=<%=payValue%>&eventValue=<%=eventValue%>"><%=i%></a>
							<%} %>
						<%}else{ %>
							<%for(int i =minPage; i<=maxPage; i++){ %>
							<%if(maxPage > cafeList.get(0).getTotalnum())break; %>
							<a href="FindGoogle.jsp?pageNum=<%=i%>&minPage=<%=minPage %>&pointValue=<%=pointValue%>&passValue=<%=passValue%>&payValue=<%=payValue%>&eventValue=<%=eventValue%>"><%=i%></a>
							<%} %>
						<%} %>
						<% if(totalPageNum>10){ %>
						<a href="FindGoogle.jsp?minPage=<%=minPage+10%>&pageNum=<%=minPage+10%>&pointValue=<%=pointValue%>&passValue=<%=passValue%>&payValue=<%=payValue%>&eventValue=<%=eventValue%>"> 다음 </a>
						<%} %>
					</div>
				<%} %>
				</div>
	...
	$(function(){
		$("#whole").click(function(){
			location.href = "FindGoogle.jsp";
		});
		$("#point").click(function(){
			location.href = "FindGoogle.jsp?pointValue=TRUE";
		});
		$("#pass").click(function(){
			location.href = "FindGoogle.jsp?passValue=TRUE";
		});
		$("#pay").click(function(){
			location.href = "FindGoogle.jsp?payValue=TRUE";
		});
		$("#event").click(function(){
			location.href = "FindGoogle.jsp?eventValue=TRUE";
		});
	});
